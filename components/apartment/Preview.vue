    ...
    <div class="card h-100">
        <div class="row">
            <div class="col-12" :class="{ 'col-md-6': isVerticalLayout }">
                <div v-if="props.photos.length !== 0" ref="sliderContainer" :id="`sliderApartmentPreview${props.id}`"
                    class="sliderApartmentPreview carousel slide">
                    <div class="carousel-indicators">
                        <button v-for="(photo, index) in props.photos" :key="index" type="button"
                            :data-bs-target="`#sliderApartmentPreview${props.id}`" :data-bs-slide-to="index"
                            :aria-current="index === 0" :aria-label="`Slide ${index + 1}`"
                            :class="{ 'active': index === 0 }">
                        </button>
                    </div>
                    <div class="carousel-inner">
                        <div v-for="(photo, index) in props.photos" :class="{ 'active': index === 0 }"
                            class="carousel-item">
                    ...
                    <div class="carousel-split position-absolute w-100 h-100 top-0 d-flex">
                        <div @mouseover="toggleSlide(index)" v-for="(photo, index) in props.photos" :key="index"
                            class="col h-100"></div>
                    </div>
                </div>
                <div v-else
                    class="slider-no-photo d-flex align-items-center justify-content-center bg-secondary text-white">
            ...
            <div class="col-12" :class="{ 'col-md-6 d-flex align-items-center': isVerticalLayout, }">
                <div class="card-body">
                    <div class="d-flex justify-content-between">
                        ...
                        <div class="dropdown">
                            <button type="button" class="btn btn-extend" data-bs-toggle="dropdown"
                                aria-expanded="false">
                                <ClientOnly>
                            ...
                            <ul class="dropdown-menu">
                                <li>
                                    <NuxtLink :to="`/apartments/edit/${props.id}`" class="dropdown-item">Редактировать</NuxtLink>
                                </li>
                                <li><a class="dropdown-item" href="#">В архив</a></li>
                    ...
                    <div>
                        <button @click="navigateToPage" type="button" class="btn btn-primary">{{ props.price }}₽ за
                            сутки</button>
                    </div>
                </div>
            </div>
